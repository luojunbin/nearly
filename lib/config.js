...
var parser = exports.parser = {
    nrSplit: function nrSplit(str) {
        ...
        var storeName = _str$split2[0];
        var dispatcherName = _str$split2[1];


        return { storeName: storeName, dispatcherName: dispatcherName };
    },
    nrImport: function nrImport(storeName) {
        var realName = storeName.split('#')[0];

        return require('./' + realName + '.js');
    },
    nrTarget: function nrTarget(mod, functionName) {
        if (mod[functionName]) {
